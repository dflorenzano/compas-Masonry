...
* Added `MasonrySession.refresh_problem_supports` and a `RefreshSupports` operation to `Problem_create_options`. Supports are copied onto a problem at creation and into each boundary condition at registration, so editing them afterwards silently left every problem holding the old set. `Model_supports_options` now detects that and offers to refresh. Prescribed displacements are preserved.
* Added **BC kinds** — `Gravity | Loads | Displacements | Mixed` — chosen with the name in `Problem_createbc_options` and enforced by `Problem_addload_options` and `Problem_displacements_options`. Held session-side (`session.bc_kinds`), keyed by index and reindexed when a BC is deleted, since compas_dem's `BoundaryCondition` has no such field.
...
* **`LoadCase` → `BoundaryCondition`**, following the compas_dem rename (`a6454c6`, no compatibility alias): `compas_masonry/loadcases.py` → `boundaryconditions.py` (`bc_name`, `bc_labels`), the session's layer API (`bc_layer`, `ensure_/clear_/delete_bc_layers`, `choose_bc(s)`, `draw_bc`, `draw_problem_bcs`), and the layer prefix `LC<n>_` → `BC<n>_`. `CM_Problem_createloadcase_options.py` → `CM_Problem_createbc_options.py`, and `CM_Problem_boundaryconditions_options.py` → `CM_Problem_displacements_options.py`, whose old name now collided with the BC concept. Existing session JSON holding `LoadCase` dtypes no longer loads.
...
* `Problem_solve_options` checks contacts, solver, contact law, supports and material densities before starting, and resolves `ipopt` first, so a failure names the command to run instead of surfacing a traceback.
* `Results_show_options` draws per stored result set, with a Forces / Displaced / Both mode that defaults to **Forces** when every transformation is an identity.
...
* `Problem_contactlaw_options` prints **both** phi and mu, read back from the stored contact model, since they are two views of the same thing and mu is what the solvers use.
* `Model_materialassign` now tags blocks with `material_name` as well as `material_guid`.
* `fc` → `fck` in `Model_material_options`, following compas_dem `c924950`; `Ecm` is labelled MPa (the predefined values are 20000/30000).
...
* `print_predefined` in `Model_material_options` still read the `fc` key and would have printed "-" for every material's strength.
...
* Added `commands/*_options.py`, the RhinoCommon variant of every command that takes input, plus the new commands of the dev notes refactor: `Problem_createloadcase_options`, `Problem_addload_options`, `Problem_solve_options`, `Results_show_options`, `Masonry_export_options`, `Masonry_import_options`.
* Added a temporary face-index helper to `Problem_addload_options`: the faces of the selected block are labelled with their index while the surface load face is picked, and the labels are always removed afterwards.
* Added a session-level export/import pair (`Masonry_export_options` / `Masonry_import_options`) covering the model, every problem with its load cases, the active problem and the display settings in one JSON file.
...
* `Problem_create_options` asks for the problem name as a named option seeded with the next free `Problem_<n>` (no string prompt on the first window), and gained a `Delete` operation, which is where the layer renumbering happens.
* `Problem_boundaryconditions_options` now edits a load case, and draws a prescribed displacement as an arrow and a prescribed rotation as a circle around its axis — no displaced copy of the geometry (that is `Results_show`).
* `Model_material_options` prints every predefined material with its values as a table before the pick, offers a `Custom` entry in the same list that leads into the custom generation, and offers `Back` on the property window.
* `Problem_solve_options` draws nothing: it stores one result per solved load case on the session, and `Results_show_options` is what puts displaced geometry in the document.
...
* `MasonrySession.clear_model` now prunes stale guids first, clears each scene object (`obj.clear()`) before removing it, and matches `EdgeContact` / `VertexContact` alongside `FrictionContact` — the same three corrections applied to `Model_contacts_options`, so every teardown path behaves the same way.
* `Model_contacts_options` now clears contact/graph scene objects (`obj.clear()`) before removing them, and matches `EdgeContact` and `VertexContact` as well as `FrictionContact`. Those two are plain `Data` subclasses, **not** `FrictionContact` subclasses, so `find_all_by_itemtype(FrictionContact)` left their scene objects behind while `clear_layer` deleted the geometry under them — the exact way the redraw above got poisoned. The same pattern is still present in the frozen original `CM_Model_contacts.py`, which is now protected by the session-level prune rather than fixed in place.
...
* Removed `Problem_contactmodel_options`, renamed to `Problem_contactlaw_options` (dev notes: a problem holds one solver and one contact law).
* Removed `Problem_loads_options`, superseded by `Problem_addload_options` (loads belong to a load case).
...
* `LoadCase.add_displacement(block_index, dx=, dy=, dz=)` is **per component**, where `None` means the DOF is unconstrained — which is not the same as prescribing 0.0. `Problem_boundaryconditions_options` mirrors that with a Prescribed/Free toggle per axis, and the value option for an axis only appears when that axis is constrained.
...
* `Solver` now offers LMGC90 / CRA / PRD / **BLA** / RBE (no DPRD). `Problem_solver_options` follows, with the real parameters per solver; the dict-shaped `non_linear_params` / `non_associative_params` are left at their compas_dem defaults since they have no command line widget.
