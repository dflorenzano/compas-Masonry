...
def test_solve_checks_compas_cra_takes_external_loads():
    """compas_dem hands `loads=` to cra_solve; released compas_cra has no such
    parameter and dies mid-solve with a TypeError naming neither package."""
    solve = load_command(command_path("Problem_solve"), "solve")
    assert solve.cra_accepts_loads() in (True, False)
...
def problem():
    ...
    return Problem(BlockModel(), name="ULS")


def test_a_group_is_carried_by_the_condition_name(problem):
    """compas_dem has no container to group boundary conditions — a Problem holds
    one flat list — so the group is the condition's own name."""
    from compas_masonry.boundaryconditions import group_name, groups

    problem.add_point_load_at_face(block=1, face=2, force=[0, 0, -5000], name="Load_1")
    problem.add_point_load_at_vertex(block=3, vertex=0, force=[0, 0, -1000], name="Load_1")
    problem.add_surface_load(block=4, face=1, traction=[0, 0, -200], name="Wind")

    assert {k: len(v) for k, v in groups(problem, "load").items()} == {"Load_1": 2, "Wind": 1}
    assert group_name(problem.loads[0]) == "Load_1"


def test_grouping_survives_serialization(problem):
    """`name` is not in __data__, but compas writes it in the Data envelope — so
    the grouping round-trips. If it did not, every reopened session would collapse
    into one group per kind."""
    import compas

    from compas_masonry.boundaryconditions import groups

    problem.add_point_load_at_face(block=1, face=2, force=[0, 0, -5000], name="Load_1")
    problem.add_surface_load(block=4, face=1, traction=[0, 0, -200], name="Wind")

    back = compas.json_loads(compas.json_dumps(problem))
    assert set(groups(back, "load")) == {"Load_1", "Wind"}


def test_an_unnamed_condition_does_not_become_its_own_group(problem):
    """compas `Data.name` falls back to the CLASS NAME, so reading `name` would
    put an unnamed PointLoad in a group called "PointLoad"."""
    from compas_masonry.boundaryconditions import group_name

    problem.add_point_load_at_centroid(block=0, force=[0, 0, -1])
    assert problem.loads[0].name == "PointLoad"  # the trap this guards against
    assert group_name(problem.loads[0]) == "Load_1"
...
def test_next_group_name_counts_past_gaps(problem):
    ...
    problem.add_point_load_at_centroid(block=0, force=[0, 0, -1], name="Load_1")
    problem.add_point_load_at_centroid(block=1, force=[0, 0, -1], name="Load_3")
