...
"""Masonry_import_options — restore a session exported by Masonry_export.

Replaces the current session state: the model and every problem (with its load
cases) are read back, the active problem is restored, the model is drawn and
the boundary condition layers are regenerated.
...
def RunCommand():
    ...
    if not isinstance(data, dict) or "blockmodel" not in data:
        return warn("This is not a COMPAS Masonry session file. Use Model_import for a bare BlockModel.")
